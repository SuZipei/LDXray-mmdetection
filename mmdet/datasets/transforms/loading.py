...
from mmengine.fileio import get, fileio
...
class Load2ImageFromFiles(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        try:
            if self.file_client_args is not None:
                ...
                file_clientB = fileio.FileClient.infer_client(
                    self.file_client_args, imgB)
                img_bytesB = file_clientB.get(imgB)
            else:
                img_bytes = fileio.get(
                    filename, backend_args=self.backend_args)
                img_bytesB = fileio.get(
                    imgB, backend_args=self.backend_args)
            img = mmcv.imfrombytes(
                img_bytes, flag=self.color_type, backend=self.imdecode_backend)
            imgB = mmcv.imfrombytes(
                img_bytesB, flag=self.color_type, backend=self.imdecode_backend)
        ...
        # results['img'] = img 
        results['img'] = np.concatenate([img, imgB], axis=-1)
